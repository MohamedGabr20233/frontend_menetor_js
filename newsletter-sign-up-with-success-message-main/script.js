...
window.addEventListener('load', function () {
  if (window.innerWidth <= 650) {
    ...
    sectionImg.innerHTML = mobileImg;
  } else {
    //desktop devices
    sectionImg.innerHTML = desktopImg;
  }
});
...
brnValid.addEventListener('click', () => {
  const email = inputEmail.value.trim();
  //check if email has more than one @

  const emailCheck = email.split('').filter((email) => email == '@');

  if (emailCheck.length > 0 && emailCheck.length < 2 && email !== '') {
